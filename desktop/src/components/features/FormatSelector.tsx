...
export function FormatSelector({ metadata, onDownload, downloading }: FormatSelectorProps) {
  ...
  const videoQualities = [...new Set(metadata.formats.map(f => f.quality).filter(q => q))];
    ...
    <div className="w-full glass-panel-premium rounded-2xl p-6 light-bleed space-y-5">
      <div ref={tabContainerRef} className="relative bg-surface-container-highest rounded-full p-1 w-max flex">
        ...
        <button
          ...
          className={`relative z-10 px-5 py-1.5 rounded-full font-bold text-[11px] tracking-wider spring-transition ${tab === 'video' ? 'text-on-primary-container' : 'text-on-surface-variant hover:text-on-surface'}`}
        >
          Video MP4
        </button>
        <button
          ...
          className={`relative z-10 px-5 py-1.5 rounded-full font-bold text-[11px] tracking-wider spring-transition ${tab === 'audio' ? 'text-on-primary-container' : 'text-on-surface-variant hover:text-on-surface'}`}
        >
          Audio MP3
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 max-w-[240px]">
        {(tab === 'video' ? videoQualities : audioQualities).map((quality) => {
          const isAvailable = tab === 'audio' || true;
          ...
          return (
            <button
              ...
              className={`py-2.5 rounded-lg font-bold text-[11px] tracking-wider spring-transition ${selectedQuality === quality
